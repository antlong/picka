...
def isodate(start=1900, end=2010):
    ...
    :type end: int
    """
    start = datetime.datetime.toordinal(calendar.datetime.date(start, 1, 1))
    end = datetime.datetime.toordinal(calendar.datetime.date(end, 12, 31))
...
def pattern_next(pattern, tester=None, debug=False):
...
def pattern_curr(pattern, tester=None):
...
def get_in_group(rowkey, select=None):
    ...
    if isinstance(data, list):
        ...
        for each in select:
            retlist.append(
                data[each if -len(data) < each < len(data) else -len(data) + 1 if each < 0 else len(data) - 1])
